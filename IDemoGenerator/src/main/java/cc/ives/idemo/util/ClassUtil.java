...
import cc.ives.idemo.BuildConfig;
import dalvik.system.DexFile;

/**
 * Scanner, find out class with any conditions, copy from google source code.
 * 基于阿里arouter的com.alibaba.android.arouter.utils.ClassUtils
...
public class ClassUtil {
    ...
    private static final ThreadPoolExecutor threadPool = new ThreadPoolExecutor(0, 2, 2000, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>());
...
//        通过是否存在sp中的multidex.version是不准确的，因为从低版本升级上来的用户，是包含这个sp配置的
    ...
    private static List<String> tryLoadInstantRunDexFile(ApplicationInfo applicationInfo) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && null != applicationInfo.splitSourceDirs) {
            ...
            IDLog.d(TAG, "Found InstantRun support");
        } else {
            try {
                ...
                IDLog.d(TAG, "InstantRun support error, " + e.getMessage());
            }
        }
    ...
    private static boolean isVMMultidexCapable() {
        ...
        IDLog.d(TAG, "VM with name " + vmName + (isMultidexCapable ? " has multidex support" : " does not have multidex support"));
